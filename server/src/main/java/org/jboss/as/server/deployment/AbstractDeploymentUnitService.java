...
public abstract class AbstractDeploymentUnitService implements Service<DeploymentUnit> {
    ...
    public static final class DeploymentServiceListener extends AbstractServiceListener<Object> {
        ...
        private DeploymentStatus calculateNewStatus() {
            return startFailedServices.isEmpty() && servicesMissingDependencies.isEmpty() ? DeploymentStatus.OKAY : DeploymentStatus.FAILED;
        }

        public synchronized void explainStatus() {
            DeploymentStatus oldStatus = previous;
            final DeploymentStatus newStatus = calculateNewStatus();
            if (oldStatus != newStatus) {
                ...
                if (hasFailed || hasMissing) {
                    ...
                    msg.append("Deployment '").append(deploymentName).append('\'');
                    if (hasFailed) {
                        msg.append(" has failed services");
                        if (hasMissing) {
                            msg.append(" and");
                        }
                    }
                    if (hasMissing) {
                        msg.append(" has services missing dependencies");
                    }
                    if (hasFailed) {
                        msg.append("\n\tFailed services:");
                    ...
                    if (hasMissing) {
                        ...
                        for (ServiceController<?> service : servicesMissingDependencies) {
                            ...
                            /* todo
                            for (ServiceName name : service.getMissingDependencies()) {
                                msg.append("\n\t\t\t").append(name);
                            }
                            */
                        }
